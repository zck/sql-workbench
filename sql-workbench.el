...
(defun swb-insert-sliced-image (image)
  "Insert IMAGE into current buffer at point.
IMAGE is displayed by inserting a space into the current buffer
with a `display' property whose value is the image."
  ...
  (-let* ((y 0.0)
          ...
          (dy (/ 1.0001 (/ (float height) (default-font-height)))))
    (goto-char (point-min))
    (forward-line 3)
    (while (< y 1.0)
      (let ((empty-line (save-excursion
        ...
        (let ((start (point)))
          ...
          (add-text-properties start (point)
                               `(display ,(list (list 'slice 0.0 y 1.0 dy) image)
                                         rear-nonsticky (display))))
        (setq y (+ y dy))
        (if empty-line (insert "\n") (forward-line))))))
